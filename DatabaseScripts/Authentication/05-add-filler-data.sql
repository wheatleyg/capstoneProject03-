...
-- Insert fact_tags for Space genre
INSERT INTO `fact_tags` (GenreId, AvailableTags)
SELECT @space_genre_id, JSON_ARRAY('space', 'astronomy', 'solar system', 'planets', 'stars', 'galaxy', 'universe', 'cosmos', 'nebula', 'black hole')
    WHERE @space_genre_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM `fact_tags` WHERE GenreId = @space_genre_id);

-- Insert fact_tags for Cats genre
INSERT INTO `fact_tags` (GenreId, AvailableTags)
SELECT @cat_genre_id, JSON_ARRAY('cat', 'cats', 'kitten', 'kittens', 'feline', 'felines', 'pet', 'pets', 'meow', 'whiskers')
    WHERE @cat_genre_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM `fact_tags` WHERE GenreId = @cat_genre_id);
